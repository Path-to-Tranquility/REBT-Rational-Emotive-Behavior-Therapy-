...
function setup(supported = true) {
  ...
  function say(text) {
    ...
    latest.onresult({resultIndex: 0, results: [result]});
    latest.onend();
  }
  return { run, fields, elements, flush, say, saves: () => saves, session: () => latest };
}

test('narrates, reads back, redoes, and requires final permission before CSV save', async () => {
  const app = setup();
